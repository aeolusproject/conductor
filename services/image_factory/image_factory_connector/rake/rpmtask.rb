  ...
  class RpmTask < PackageTask
    ...
    def init(rpm_spec)
      ...
      File.open(rpm_spec, "r") { |f|
        ...
        # Now we can expand %{name}, %{version}, %{release}, and %{arch}:
        %w(name version release arch).each do |var|
          contents.gsub!("%{#{var}}", instance_variable_get('@'+var)) if instance_variable_get('@'+var).size>0
        end
        @sources = contents.scan(/\nSource\d+: .*/).collect{|line| line.split.last.strip}
      }
      super(@name, @version)
    ...
    def define
      ...
      file rpm_file => [:package, "#{@topdir}/SOURCES", "#{@topdir}/SPECS"] do |t,args|
        ...
        @sources.each do |source|
          ...
          search_path = [@topdir, package_dir, "#{package_dir}/.."]
          dir = search_path.select{|path| File.exists?("#{path}/#{source}")}.first
