...
class LegacyTemplate < ActiveRecord::Base
  ...
  has_many :legacy_images, :dependent => :destroy
  has_many :instances
  ...
  after_create :ensure_legacy_assembly
  ...
  def providers
    ...
    LegacyProviderImage.all(
      :include => [:legacy_image, :provider],
      :conditions => {:legacy_images => {:legacy_template_id => self.id}}
    ).map {|p| p.provider}
  end

  def self.find_or_create(id)
    id ? LegacyTemplate.find(id) : LegacyTemplate.new
  end
  ...
  def ensure_legacy_assembly
    self.legacy_assemblies.create!({
      :name => self.name,
      :architecture => self.architecture,
      :owner => self.owner
    }) unless self.legacy_assemblies.count > 0
  end
end
