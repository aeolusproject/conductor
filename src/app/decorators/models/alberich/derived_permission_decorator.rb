...
Alberich::DerivedPermission.class_eval do
  class << self
  ...
  belongs_to :hardware_profile,       :class_name => "hardwareProfile",
                                      :foreign_key => "permission_object_id"
  belongs_to :base_permission_object, :class_name => "Alberich::BasePermissionObject",
                                      :foreign_key => "permission_object_id"
  ...
  PROFILE_PRESET_FILTERS_OPTIONS = [
    {:title => "permissions.global",
     ...
     :where => {"permission_object_type" => "Alberich::BasePermissionObject"}},
  ...
  def self.apply_search_filter(search)
    ...
    if search
      ...
      where_str = "lower(alberich_entities.name) LIKE :search OR lower(alberich_roles.name) LIKE :search"
