...
class AddLegacyTemplateCreatorRole < ActiveRecord::Migration


  def self.up
    unless Role.all.empty?
      Role.transaction do
        role_name = "LegacyTemplate Creator"
        ...
        ["view","use","create"].each do |action|
          Privilege.create!(:role => role, :target_type => LegacyTemplate.name,
                            :action => action)
        end
      end
      settings = {"self_service_default_template_obj" => BasePermissionObject.general_permission_scope,
        "self_service_default_template_role" => Role.find_by_name("LegacyTemplate Creator"),
